...
- SMoL
  - 빛 번짐 synthetic 데이터 생성, contrastive alignment, domain adaptation, line detection 복원을 한 흐름으로 시각화
  - 학습이 진행될수록 positive similarity는 올라가고 domain gap은 줄어들며 glare 환경 line detection이 안정화됨
  - 우하단 `Exit` 버튼을 통해 허브로 복귀
...
`SMoL`은 빛 번짐과 반사광이 있는 환경에서도 차선 검출이 무너지지 않도록 학습시키는 과정을 시각화한 프로젝트다.
...
- baseline과 SMoL 추론을 나란히 두고, 빛 노이즈 상황에서 line detection이 어떻게 복원되는지 비교한다.
