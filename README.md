...
- SMoL
  - 동일한 DeepRacer 스타일 트랙 두 개 위에서 baseline과 SMoL 차량을 10초 주기로 비교 주행
  - 특정 glare 구간에서 차량이 현재 인식하는 lane window를 직접 보여주고, baseline은 탈선하며 SMoL은 lane을 유지
  - 우하단 `Exit` 버튼을 통해 허브로 복귀
...
- 코로나 시기 공간 제약, 짧은 준비 기간, 역할 분담, 120개 이상의 아이디어 스케치 같은 실제 프로젝트 맥락도 함께 담았다.
- 3-leg 회전형 초기안에서 2-leg 교차 고정형 최종안으로 바뀐 이유와, T자형 다리 / 5mm 두께 / 전자석 지지 구조 같은 3D 프린팅 설계 판단을 같이 설명한다.
- 역기전력, 다이오드 연기, 정지마찰 문제를 고무줄과 유격으로 우회한 시행착오도 기록한다.

## SMoL 설명

`SMoL`은 빛 번짐과 반사광이 있는 환경에서도 차선 검출이 무너지지 않도록 학습시키는 과정을, 실제 주행 비교 화면까지 포함해 시각화한 프로젝트다.

- StyleGAN / CycleGAN 계열 아이디어를 참고한 glare 데이터 생성, real-to-synthetic augmentation, contrastive alignment, domain adaptation을 한 흐름으로 배치한다.
- 원본 이미지와 노이즈 이미지에 같은 GT를 주어 line mask가 illumination보다 우선되게 학습되는 그림을 보여준다.
- 동일한 DeepRacer 스타일 폐곡선 트랙 두 개와 동일한 차량 두 대를 놓고, glare 구간에서 baseline은 탈선하고 SMoL은 lane을 유지하는 장면을 10초 루프로 보여준다.
- 차량 앞쪽에 표시되는 선이 현재 모델이 인식하는 lane window이며, 이 선이 glare 구간에서 어떻게 달라지는지를 직접 비교한다.
- 추후 실시간 적용을 위한 MobileNet / EfficientNet 계열 경량 백본 방향도 함께 제시한다.
